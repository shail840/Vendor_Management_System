...
from .models import Historical_performance, Purchase_Order
...
def performace_update(sender, instance, **kwargs):
    ...
    completed_pos = Purchase_Order.objects.filter(
        vendor=vendor, status='Completed')
    total_completed_pos = completed_pos.count()

    # on-time delivery rate calculation in percentage

    on_time_delivery = completed_pos.filter(
    vendor=vendor, delivery_date__lte=timezone.now()).count() / total_completed_pos * 100 if total_completed_pos > 0 else 0.0


    # average quality rating calculation(1 to 5 rating)

    total_rating = Purchase_Order.objects.filter(vendor=vendor, quality_rating__isnull=False).aggregate(
    ...
    issued_pos_count = Purchase_Order.objects.filter(vendor=vendor).count()
    fulfillment_rate = total_completed_pos / \
        issued_pos_count*100 if issued_pos_count else 0.0

    # Update Vendor model
    vendor.on_time_delivery = on_time_delivery
    vendor.quality_rating_avg = average_quality_rating
    vendor.average_response_time = round(average_response_time.total_seconds() / 60 if average_response_time else 0.0, 2)
    ...
    with transaction.atomic():
        update_historical_performance = Historical_performance.objects.create(
            vendor=vendor,
            date=timezone.now(),
            on_time_delivery=on_time_delivery,
            quality_rating_avg=average_quality_rating,
            average_response_time=round(average_response_time.total_seconds() / 60 if average_response_time else 0.0, 2),
